...
// NEW FUNCTION: Get a simplified list of customers for dropdowns
/**
 * @desc    Get a simplified list of customers (e.g., for dropdowns)
 * @route   GET /api/customers/list
 * @access  Private (Admin, Manager)
 */
const getCustomerList = asyncHandler(async (req, res) => {
    const companyId = req.user.company;
    // Fetch only contactPersonName and _id, and convert to plain JS objects for efficiency
    // Assuming 'contactPersonName' is the field you want for display in the dropdown
    const customerList = await Customer.find({ company: companyId }, 'contactPersonName _id').lean(); 
...
module.exports = {
    ...
    bulkUploadCustomers,
    getCustomerStats,
    getCustomerList, // ⭐ **THIS IS THE KEY CHANGE: Export the new function!**
};